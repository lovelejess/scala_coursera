...
class HuffmanSuite extends FunSuite {
  ...
  test("createCodeTree ") {
    val charsList = List('a','b','c','d','a','d','a')
    assert(createCodeTree(charsList) === List(Fork(Leaf('a',3), Fork(Leaf('d',2), Fork(Leaf('b',1), Leaf('c',1), List('b','c'), 3),List('d','b','c'), 4), List('a','d','b','c'), 7)))
  }
